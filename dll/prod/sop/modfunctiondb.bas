...
Function getHPP(ByVal kode_barang As String, ByVal qty_stok As Double, ByVal qty_use As Double) As Double
    ...
    Do While Not RS.EOF
        ...
        End If
        RS.MoveNext
        'MsgBox RS!kodebarang & " - " & RS!nobeli & RS!harga
    Loop
    OBJ.Close
    getHPP = hpp
    Exit Function
...
Public Function getnobpb(ByVal tanggal As String)
On Error GoTo Err_handler:
    Dim strformat As String
    strformat = Format(tanggal, "yymm")
    
    Dim str99 As String
    Dim no As String
    Set OBJ = New ADODB.Connection
    OBJ.Open dsn
    SQL = "select top 1 nobpb from am_bpbhdr "
    SQL = SQL + "where nobpb like 'PHL0-' + '" + strformat + "%' order by nobpb desc"
    ...
    If Len(str99) = 1 Then no = "PHL0-" & strformat & "0" & str99
    If Len(str99) = 2 Then no = "PHL0-" & strformat & str99
