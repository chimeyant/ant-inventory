...
Private Sub cmdclear_Click()
    ...
    Crystal.ReportFileName = AppPath & "\reports\gl\tables\listbank.rpt"
...
Private Sub cariunit()
    ...
    OBJ.Open dsn
    SQL = "select * from am_bank where kode = '" & txtkode1 & "'"
    Set RST = OBJ.Execute(SQL)
    If Not RST.EOF Then
        OBJ.Close
        Exit Sub
...
Private Sub cariunit1()
    ...
    OBJ.Open dsn
    SQL = "select * from am_bank where kode = '" & txtkode2 & "'"
    Set RST = OBJ.Execute(SQL)
    If Not RST.EOF Then
        OBJ.Close
        Exit Sub
