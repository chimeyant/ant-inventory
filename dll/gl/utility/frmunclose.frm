...
Begin VB.Form frmunclose 
   ...
   Begin VB.Frame Frame3 
      Caption         =   "Periode After Unclosing"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
...
Private Sub CmdSubmit_Click()
    ...
    "Continue Unclosing ?", vbYesNo + vbQuestion, "Question") = vbNo Then Exit Sub
    
    If MsgBox("Accept This Periode Before Closing ?", vbYesNo + vbQuestion, "Question") = vbNo Then Exit Sub
    
    If date1 >= date4 Then
        MsgBox "Periode Before Process Can't Bigger Then Periode On Process.", vbInformation, "Information"
        Exit Sub
    End If
    
    If date2 <= date1 Then
        MsgBox "Periode Akhir Before Process Can't Smaller Then Periode Awal Before Process.", vbInformation, "Information"
        Exit Sub
    End If
    ...
    Set RST = OBJ.Execute(SQL)
    If Not RST.EOF Then
        If RST!awaltahun = Format(RST!tglawal, "yyyy") Then
            If str2 = RST!awalbulan Then
                MsgBox "This Is Start of Periode, User Can Not Unclose.", vbInformation, "Information"
        ...
        End If
    Else
        MsgBox "Cannot continue user must define Company ID.", vbInformation, "Information"
    ...
    If Not RST.EOF Then
        ...
        If Not RST1.EOF Then
            MsgBox "Unclosing Will Not Continue Until The Transaction Are Unposted", vbInformation, "Information"
            OBJ.Close
            OBJ1.Close
            Exit Sub
        End If
    ...
    If Val(str2) = "1" Then
        If MsgBox("Continue Un End Year Proces ?", vbYesNo + vbQuestion, "Question") = vbNo Then
            OBJ.Close
            Exit Sub
        End If
        SP.ActiveConnection = dsn
        SP.CommandType = adCmdStoredProc
        SP.CommandText = "gl_unendyear_x"
        vsp(0) = txtarea1
        vsp(1) = txtperiode
        vsp(2) = tanggal1
        vsp(3) = tanggal2
        ...
        Set SP = Nothing
    Else
        If MsgBox("Continue Un End Month Proces ?", vbYesNo + vbQuestion, "Question") = vbNo Then
            OBJ.Close
            Exit Sub
        End If
        SP.ActiveConnection = dsn
        SP.CommandType = adCmdStoredProc
        SP.CommandText = "gl_unendmonth_x"
        vsp(0) = txtarea1
        vsp(1) = txtperiode
        vsp(2) = tanggal1
        vsp(3) = tanggal2
        ...
        Set SP = Nothing
    End If
    SQL = "delete from gl_transaksi where kdcomp = '" & txtarea1 & "' and kdtrx = 'CP' and notrx = 'CP" & Format(date3, "YY") & Format(date3, "MM") & "'"
    Set RST = OBJ.Execute(SQL)
    OBJ.Close
    MsgBox "Unclosing Complete", vbInformation, "Information"
    Unload Me
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    OBJ.Open dsn
    SQL = "delete from toogle where comp_id = '" & GetTheComputerName & "' and task = 'Unclosing'"
    Set RST = OBJ.Execute(SQL)
    OBJ.Close
End Sub
